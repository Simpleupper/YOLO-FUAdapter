...
from ultralytics.nn.ppr_ncm import CenterPeripheralRefiner, PPR, NCM
...
def parse_model(d, ch, verbose=True):
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in {ContextAwareDIP, CSAContextAdjust, ContextScaleDIP}:
            # Context-aware image adjustment keeps input channels and spatial size unchanged.
            c1 = ch[f]
            args = [c1, *args]
            c2 = c1

        elif m is DySample:
            # ELA-YOLO DySample: YAML [channels, scale, style]; channels are inferred from input.
            c1 = ch[f]
            args = [c1, *args[1:]] if len(args) >= 1 else [c1]
            c2 = c1

        elif m is SelectiveFusion:
            # ELA-YOLO selective fusion takes two inputs, so f is a list.
            if not isinstance(f, (list, tuple)) or len(f) != 2:
                raise TypeError(f"SelectiveFusion expects two input feature maps, got f={f}")
            c_high, c_low = ch[f[0]], ch[f[1]]
            c_out = args[2] if len(args) > 2 else c_low
            c_out = make_divisible(min(c_out, max_channels) * width, 8)
            args = [c_high, c_low, c_out]
            c2 = c_out
            ...
            if m in repeat_modules:
                ...
                n = 1
            if m in {C3k2, C3k2_EMSC, C3k2_TripletAttention}:  # for M/L/X sizes
                legacy = False
                if scale in "mlx":
                    args[3] = True
            if m is A2C2f:
            ...
            if m is C2fCIB:
                legacy = False
        # elif m in frozenset({PPR, NCM, CenterPeripheralRefiner}):
        #     if not isinstance(f, (list, tuple)):
        #         raise TypeError(f"{m.__name__} expects multi-scale inputs from a list of layer indices, got f={f}")
        #     in_ch = [ch[x] for x in f]
        #     if len(set(in_ch)) != 1:
        #         raise ValueError(
        #             f"{m.__name__} requires all input scales to share the same channel dimension, got {in_ch}. "
        #             "Please project P3/P4/P5 to a common channel count before this module."
        #         )
        #     expected_dim = args[1] if m in {PPR, CenterPeripheralRefiner} else args[0]
        #     if expected_dim != in_ch[0]:
        #         raise ValueError(
        #             f"{m.__name__} feature_dim/channel mismatch: module expects {expected_dim}, "
        #             f"but incoming features have channels {in_ch[0]}"
        #         )
        #     c2 = in_ch
            ...
            # 对 feature_dim 做和 Conv 一致的 width scaling
            if m in {PPR, CenterPeripheralRefiner}:
            ...
            {Detect, WorldDetect, YOLOEDetect, Segment, YOLOESegment, Pose, OBB, ImagePoolingAttn, v10Detect, ELADetect, ELDynamicDetect, SFWDynamicDetect, YOLOV5ImprovementDetect, CSAScaleAwareDetect}
        ):
            args.append([ch[x] for x in f])
            if m is Segment or m is YOLOESegment:
                args[2] = make_divisible(min(args[2], max_channels) * width, 8)
            if m in {Detect, YOLOEDetect, Segment, YOLOESegment, Pose, OBB, ELDynamicDetect, SFWDynamicDetect, YOLOV5ImprovementDetect, CSAScaleAwareDetect}:
                m.legacy = legacy
