...
__all__ = "OBB", "Classify", "Detect", "ELADetect", "ELDynamicDetect", "ELDyHeadBlock", "YOLOV5ImprovementDetect", "CSAScaleAwareDetect", "Pose", "RTDETRDecoder", "Segment", "YOLOEDetect", "YOLOESegment", "v10Detect"
...
class ELADetectBranch(nn.Module):
    """Lightweight branch used by ELA-YOLO detection head.

    It first uses a 1x1 Conv as an MLP-like channel projection, then applies dilated depthwise convolution and pointwise
    convolution, and finally adds a residual path before the prediction layer.
    """
    def __init__(self, c1: int, c2: int, out_channels: int, dilation: int = 2):
        super().__init__()
        self.mlp = Conv(c1, c2, 1, 1)
        self.dw_dilated = nn.Conv2d(c2, c2, 3, 1, padding=dilation, dilation=dilation, groups=c2, bias=False)
        self.bn = nn.BatchNorm2d(c2)
        self.act = nn.SiLU()
        self.pw = Conv(c2, c2, 1, 1)
        self.pred = nn.Conv2d(c2, out_channels, 1)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        x = self.mlp(x)
        y = self.pw(self.act(self.bn(self.dw_dilated(x))))
        return self.pred(x + y)


class ELADetect(Detect):
    """ELA-YOLO lightweight decoupled detection head.

    This replaces YOLOv8's heavier decoupled convolutional head with an MLP-like projection followed by separate
    dilated-convolution and pointwise-convolution branches for box regression and classification.
    """
    def __init__(self, nc: int = 80, ch: tuple = ()):  # ch=(P3,P4,P5 channels)
        super().__init__(nc, ch)
        c2 = max((16, ch[0] // 4, self.reg_max * 4))
        c3 = max(ch[0], min(self.nc, 100))
        self.cv2 = nn.ModuleList(ELADetectBranch(x, c2, 4 * self.reg_max, dilation=2) for x in ch)
        self.cv3 = nn.ModuleList(ELADetectBranch(x, c3, self.nc, dilation=2) for x in ch)
        self.dfl = DFL(self.reg_max) if self.reg_max > 1 else nn.Identity()
        if self.end2end:
            self.one2one_cv2 = copy.deepcopy(self.cv2)
            self.one2one_cv3 = copy.deepcopy(self.cv3)
    ...
    #         a.pred.bias.data[:] = 1.0
    #         b.pred.bias.data[: m.nc] = math.log(5 / m.nc / (640 / s) ** 2)
    #     if self.end2end:
    #         for a, b, s in zip(m.one2one_cv2, m.one2one_cv3, m.stride):
    #             a.pred.bias.data[:] = 1.0
    #             b.pred.bias.data[: m.nc] = math.log(5 / m.nc / (640 / s) ** 2)
    def bias_init(self):
        """Initialize ELADetect biases."""
        m = self
    
        def last_conv(module):
            convs = [x for x in module.modules() if isinstance(x, nn.Conv2d)]
            return convs[-1] if len(convs) else None
    
        for reg_branch, cls_branch, s in zip(m.cv2, m.cv3, m.stride):
            reg_out = last_conv(reg_branch)
            cls_out = last_conv(cls_branch)
    
            if reg_out is not None and reg_out.bias is not None:
                reg_out.bias.data[:] = 1.0
    
            if cls_out is not None and cls_out.bias is not None:
                cls_out.bias.data[: m.nc] = math.log(5 / m.nc / (640 / s) ** 2)
    
        print("ELADetect bias_init done.")


# ==================== EL-YOLOv8 dynamic head ====================

class ELDyHeadBlock(nn.Module):
    """Lightweight dynamic-head block with spatial, channel and scale-aware modulation.

    This is a stable Ultralytics-compatible implementation for EL-YOLOv8. It applies lightweight
    spatial depthwise convolution, channel attention, and a learnable residual scale before the
    standard decoupled Detect branches.
    """
...
class ELDynamicDetect(Detect):
    """EL-YOLOv8 dynamic detection head.

    The head first refines P3/P4/P5 features using lightweight dynamic attention blocks, then uses
    the standard YOLOv8 decoupled detection branches. This keeps compatibility with Ultralytics loss,
    bias initialization and export logic.
    """

    def __init__(self, nc: int = 80, ch: tuple = (), dy_depth: int = 1):
        super().__init__(nc, ch)
        self.dy_blocks = nn.ModuleList(
            nn.Sequential(*(ELDyHeadBlock(c) for _ in range(dy_depth))) for c in ch
        )

    def forward(self, x: list[torch.Tensor]) -> list[torch.Tensor] | tuple:
        x = [block(xi) for block, xi in zip(self.dy_blocks, x)]
        return super().forward(x)


class SFWDynamicDetect(ELDynamicDetect):
    """SFW-YOLO Dynamic Head for four-scale P2/P3/P4/P5 detection.

    SFW-YOLO adds a high-resolution P2 detection branch and uses a dynamic head.
    This implementation reuses the stable ELDynamicDetect refinement blocks but supports
    an arbitrary number of feature levels. The final module exposes ``use_eiou=True`` so
    the optional merged loss file can switch only this model to EIoU while keeping the
    other algorithms on their original loss.
    """

    def __init__(self, nc: int = 80, ch: tuple = (), dy_depth: int = 1):
        super().__init__(nc=nc, ch=ch, dy_depth=dy_depth)
        self.use_eiou = True
        self.is_sfw_yolo = True

    def bias_init(self):
        super().bias_init()
        print("SFWDynamicDetect bias_init done.")



class YOLOV5ImprovementDetect(Detect):
    """Detection head marker for YOLO V5-IMPROVEMENT.

    It keeps the stable Ultralytics Detect implementation while enabling SIoU
    through ``use_siou`` in the merged loss file. This avoids changing the other
    six algorithms that share the same codebase.
    """
    ...
    def bias_init(self):
        super().bias_init()
        print("YOLOV5ImprovementDetect bias_init done.")


class CSAScaleAwareDetect(Detect):
    """Detection head marker for Context and Scale-aware YOLO."""

    is_context_scale_yolo = True

    def bias_init(self):
        super().bias_init()
        print("CSAScaleAwareDetect bias_init done.")
