...
# ======================= ELA-YOLO modules: linear attention backbone + selective FPN =======================
...
def _apply_rope_bhwc(x: torch.Tensor) -> torch.Tensor:
    """Apply 2D RoPE to a BHWC tensor. Used for Q and K in ELA-YOLO."""
    b, h, w, c = x.shape
    if c < 4:
        return x
    c_even = c if c % 2 == 0 else c - 1
    x_main, x_tail = x[..., :c_even], x[..., c_even:]
    half = c_even // 2
    x_h, x_w = x_main[..., :half], x_main[..., half:]

    def rope_1d(t: torch.Tensor, length: int, axis: int) -> torch.Tensor:
        dim = t.shape[-1]
        dim_even = dim if dim % 2 == 0 else dim - 1
        t_main, t_tail = t[..., :dim_even], t[..., dim_even:]
        inv_freq = 1.0 / (10000 ** (torch.arange(0, dim_even, 2, device=t.device, dtype=t.dtype) / dim_even))
        pos = torch.arange(length, device=t.device, dtype=t.dtype)
        freqs = torch.einsum('i,j->ij', pos, inv_freq)
        emb = torch.repeat_interleave(freqs, 2, dim=-1)
        if axis == 1:  # H axis
            emb = emb.view(1, length, 1, dim_even)
        else:          # W axis
            emb = emb.view(1, 1, length, dim_even)
        out = (t_main * emb.cos()) + (_rotate_half(t_main) * emb.sin())
        return torch.cat([out, t_tail], dim=-1) if t_tail.numel() else out

    x_h = rope_1d(x_h, h, axis=1)
    x_w = rope_1d(x_w, w, axis=2)
    x_out = torch.cat([x_h, x_w], dim=-1)
    return torch.cat([x_out, x_tail], dim=-1) if x_tail.numel() else x_out


class ELALinearAttention(nn.Module):
    """Linear attention in ELA-YOLO with RoPE on Q/K, kernel mapping, DWC(V), and learnable Q/K scales."""
    def __init__(self, dim: int, num_heads: int = 8, qkv_bias: bool = True, kernel_power: float = 2.0, eps: float = 1e-6):
        super().__init__()
        ...
        self.q_scale = nn.Parameter(torch.ones(1, num_heads, 1, self.head_dim))
        self.k_scale = nn.Parameter(torch.ones(1, num_heads, 1, self.head_dim))
        self.dwc = nn.Conv2d(dim, dim, 3, 1, 1, groups=dim, bias=True)
        self.proj = nn.Conv2d(dim, dim, 1, bias=True)

    def kernel_map(self, x: torch.Tensor) -> torch.Tensor:
        # Paper Eq. (8): phi_p = ||x|| / ||x**p|| * x**p after ReLU mapping.
        x = F.relu(x) + self.eps
        xp = x.pow(self.kernel_power)
        return xp * (x.norm(dim=-1, keepdim=True) / (xp.norm(dim=-1, keepdim=True) + self.eps))

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        b, c, h, w = x.shape
        q, k, v = self.qkv(x).chunk(3, dim=1)
        v_img = v
        # BCHW -> B, heads, N, head_dim
        q = q.permute(0, 2, 3, 1).reshape(b, h, w, self.num_heads, self.head_dim).permute(0, 3, 1, 2, 4)
        k = k.permute(0, 2, 3, 1).reshape(b, h, w, self.num_heads, self.head_dim).permute(0, 3, 1, 2, 4)
        v = v.permute(0, 2, 3, 1).reshape(b, h * w, self.num_heads, self.head_dim).permute(0, 2, 1, 3)
        # apply RoPE to each head as BHWC
        q = _apply_rope_bhwc(q.permute(0, 1, 2, 3, 4).reshape(b * self.num_heads, h, w, self.head_dim)).view(b, self.num_heads, h * w, self.head_dim)
        k = _apply_rope_bhwc(k.permute(0, 1, 2, 3, 4).reshape(b * self.num_heads, h, w, self.head_dim)).view(b, self.num_heads, h * w, self.head_dim)
        q = self.kernel_map(q * self.q_scale)
        k = self.kernel_map(k * self.k_scale)
        kv = torch.einsum('bhnd,bhne->bhde', k, v)
        k_sum = k.sum(dim=2)
        z = 1.0 / (torch.einsum('bhnd,bhd->bhn', q, k_sum) + self.eps)
        out = torch.einsum('bhnd,bhde,bhn->bhne', q, kv, z)
        out = out.permute(0, 2, 1, 3).reshape(b, h, w, c).permute(0, 3, 1, 2).contiguous()
        out = out + self.dwc(v_img)
        return self.proj(out)


class ELABlock(nn.Module):
    """ELA-YOLO linear-attention block: LN + linear attention residual, then LN + MLP residual."""
    def __init__(self, c: int, num_heads: int = 8, mlp_ratio: float = 2.0, drop: float = 0.0):
        super().__init__()
        hidden = int(c * mlp_ratio)
        self.norm1 = LayerNorm2d(c)
        self.attn = ELALinearAttention(c, num_heads=num_heads)
...
class ELABottleneck(nn.Module):
    """YOLO-friendly bottleneck whose core operation is the ELA block."""
    def __init__(self, c1: int, c2: int, shortcut: bool = True, g: int = 1, e: float = 0.5, heads: int = 8):
        super().__init__()
        c_ = int(c2 * e)
        self.cv1 = Conv(c1, c_, 1, 1)
        self.ela = ELABlock(c_, num_heads=heads)
        self.cv2 = Conv(c_, c2, 1, 1)
        self.add = shortcut and c1 == c2

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        y = self.cv2(self.ela(self.cv1(x)))
        return x + y if self.add else y


class C2f_ELA(nn.Module):
    """C2f with ELA linear-attention bottlenecks, used as ELA-YOLO basic block."""
    def __init__(self, c1: int, c2: int, n: int = 1, shortcut: bool = False, g: int = 1, e: float = 0.5, heads: int = 8):
        super().__init__()
        self.c = int(c2 * e)
        self.cv1 = Conv(c1, 2 * self.c, 1, 1)
        self.cv2 = Conv((2 + n) * self.c, c2, 1)
        self.m = nn.ModuleList(ELABottleneck(self.c, self.c, shortcut, g, e=1.0, heads=heads) for _ in range(n))
...
class CBAM(nn.Module):
    """Convolutional Block Attention Module used by ELA-YOLO selective FPN."""
    def __init__(self, c1: int, reduction: int = 16, kernel_size: int = 7):
        super().__init__()
        hidden = max(c1 // reduction, 1)
        self.mlp = nn.Sequential(nn.Conv2d(c1, hidden, 1, bias=False), nn.ReLU(inplace=True), nn.Conv2d(hidden, c1, 1, bias=False))
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x = x * self.sigmoid(self.mlp(avg) + self.mlp(mx))
        avg = torch.mean(x, dim=1, keepdim=True)
        mx, _ = torch.max(x, dim=1, keepdim=True)
        return x * self.sigmoid(self.spatial(torch.cat([avg, mx], dim=1)))


class SelectiveFusion(nn.Module):
    """Selective feature pyramid fusion: CBAM refinement + semantic gating."""
    def __init__(self, c_main: int, c_skip: int, c2: int):
        super().__init__()
        self.main_att = CBAM(c_main)
        self.skip_att = CBAM(c_skip)
        self.main_proj = Conv(c_main, c2, 1, 1)
        self.skip_proj = Conv(c_skip, c2, 1, 1)
        self.gate = nn.Sequential(Conv(c2 * 2, c2, 1, 1), nn.Sigmoid())
        self.out = Conv(c2, c2, 3, 1)

    def forward(self, x: list[torch.Tensor]) -> torch.Tensor:
        x_main, x_skip = x
        x_main = self.main_proj(self.main_att(x_main))
        x_skip = self.skip_proj(self.skip_att(x_skip))
        if x_main.shape[-2:] != x_skip.shape[-2:]:
            x_main = F.interpolate(x_main, size=x_skip.shape[-2:], mode='nearest')
        gate = self.gate(torch.cat([x_main, x_skip], dim=1))
        return self.out(gate * x_skip + (1.0 - gate) * x_main)


class DySample(nn.Module):
    """DySample upsampling module with dynamic point sampling.

    This is the commonly used lp-style DySample implementation and is used to replace nn.Upsample in the ELA-YOLO neck.
    """
    def __init__(self, c1: int, scale: int = 2, style: str = 'lp', groups: int = 4, dyscope: bool = False):
        ...
        in_channels = c1 if style == 'lp' else c1 // scale ** 2
        out_channels = 2 * groups * scale ** 2 if style == 'lp' else 2 * groups
    ...
    def sample(self, x: torch.Tensor, offset: torch.Tensor) -> torch.Tensor:
        ...
        coords = torch.stack(torch.meshgrid([coords_w, coords_h], indexing='xy')).unsqueeze(1).unsqueeze(0)
...
# ======================= end of ELA-YOLO modules =======================


# ==================== EL-YOLOv8 additional modules ====================

class PConv(nn.Module):
    """Partial convolution used by FasterNetBlock.

    Only a fraction of channels are processed by a 3x3 convolution, while the remaining channels
    bypass the operation. This reduces memory access and computation while preserving spatial cues.
    """
    ...
    def __init__(self, c1: int, n_div: int = 4, k: int = 3):
        ...
        self.partial_conv3 = nn.Conv2d(self.dim_conv, self.dim_conv, k, 1, autopad(k), bias=False)
...
class EMA(nn.Module):
    ...
    This implementation follows the cross-spatial grouping idea: features are divided into groups,
    height/width context is encoded separately, and 1x1/3x3 paths are fused to enhance pixel-level
    spatial attention without reducing the final channel dimension.
    """
    ...
    def __init__(self, channels: int, factor: int = 8):
        ...
        while channels % self.groups != 0 and self.groups > 1:
            self.groups -= 1
        c_ = channels // self.groups
        self.softmax = nn.Softmax(dim=-1)
        self.agp = nn.AdaptiveAvgPool2d((1, 1))
        self.pool_h = nn.AdaptiveAvgPool2d((None, 1))
        self.pool_w = nn.AdaptiveAvgPool2d((1, None))
        self.gn = nn.GroupNorm(1, c_)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Cross-spatial weighting.
        x11 = self.softmax(self.agp(x1).reshape(b * g, -1, 1).permute(0, 2, 1))
        x12 = x2.reshape(b * g, c // g, -1)
        x21 = self.softmax(self.agp(x2).reshape(b * g, -1, 1).permute(0, 2, 1))
...
class FasterEMABlock(nn.Module):
    """FasterNet-style lightweight block fused with EMA attention."""

    def __init__(self, c1: int, mlp_ratio: float = 2.0, n_div: int = 4, ema_factor: int = 8, shortcut: bool = True):
        super().__init__()
        hidden = int(c1 * mlp_ratio)
        self.pconv = PConv(c1, n_div=n_div)
        self.ffn = nn.Sequential(
            Conv(c1, hidden, 1, 1, act=nn.ReLU()),
            Conv(hidden, c1, 1, 1, act=False),
        )
        self.ema = EMA(c1, factor=ema_factor)
        self.act = nn.SiLU()
...
class C2f_Faster_EMA(nn.Module):
    """C2f-Faster-EMA module for the EL-YOLOv8 backbone.

    It keeps the C2f split-concat topology and replaces the standard bottleneck stack with Faster-EMA blocks.
    """

    def __init__(self, c1: int, c2: int, n: int = 1, shortcut: bool = True, g: int = 1, e: float = 0.5,
                 mlp_ratio: float = 2.0, n_div: int = 4, ema_factor: int = 8):
        super().__init__()
        self.c = int(c2 * e)
        self.cv1 = Conv(c1, 2 * self.c, 1, 1)
        self.cv2 = Conv((2 + n) * self.c, c2, 1, 1)
        self.m = nn.ModuleList(
            FasterEMABlock(self.c, mlp_ratio=mlp_ratio, n_div=n_div, ema_factor=ema_factor, shortcut=shortcut)
            for _ in range(n)
        )
...
class GSConv(nn.Module):
    """GSConv used in Slim-neck.

    A standard convolution branch is combined with a cheap depthwise branch and channel-shuffled.
    """
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x = torch.cat((x1, x2), dim=1)
        b, c, h, w = x.size()
        # channel shuffle, non-inplace
        x = x.reshape(b, 2, c // 2, h, w).permute(0, 2, 1, 3, 4).reshape(b, c, h, w)
        return x


class GSBottleneck(nn.Module):
    """GSConv bottleneck used by VoV-GSCSP."""
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        y = self.conv_lighting(x)
        return y + self.shortcut(x) if self.add else y


class VoVGSCSP(nn.Module):
    """VoV-GSCSP module for Slim-neck feature fusion.

    A lightweight one-shot aggregation CSP block based on GSConv.
    """
    ...
    def __init__(self, c1: int, c2: int, n: int = 1, shortcut: bool = True, e: float = 0.5):
        ...
        self.cv1 = Conv(c1, c_, 1, 1)
        self.cv2 = Conv(c1, c_, 1, 1)
        self.gsb = nn.Sequential(*(GSBottleneck(c_, c_, shortcut=shortcut) for _ in range(n)))
...
try:
    __all__ = tuple(list(__all__) + [
        "PConv", "EMA", "FasterEMABlock", "C2f_Faster_EMA",
        "GSConv", "GSBottleneck", "VoVGSCSP",
    ])
except NameError:
    pass
# ========================= End EL-YOLOv8 modules =========================



# ========================= MSAF-YOLO modules =========================
# These modules are appended on top of the existing ELA-YOLO and EL-YOLOv8 code.
# They are designed for the MSAF-YOLO paper based on YOLOv11:
# SPDConv + C3k2_EMSC + CAFM + C3k2_TripletAttention.

class SPDConv(nn.Module):
    """Space-to-Depth convolution used by MSAF-YOLO.

    For downsampling layers (s=2), spatial information is rearranged into the channel
    dimension and then processed by a non-strided convolution. For s=1, it behaves as
    a normal Conv wrapper, which keeps the module compatible with the Ultralytics parser.
    """
    ...
    def __init__(self, c1: int, c2: int, k: int = 3, s: int = 1, p: int | None = None, g: int = 1, act: bool = True):
        ...
        self.s = int(s)
        if self.s == 2:
            self.conv = Conv(c1 * 4, c2, k, 1, p, g, act=act)
        else:
            self.conv = Conv(c1, c2, k, self.s, p, g, act=act)

    @staticmethod
    def space_to_depth(x: torch.Tensor) -> torch.Tensor:
        # Pad odd spatial sizes to keep the operation robust for non-even inputs.
...
class ZPool(nn.Module):
    """Z-Pool used by Triplet Attention: concatenate max-pool and avg-pool along channels."""
...
class TripletAttentionGate(nn.Module):
    """A single spatial attention gate used inside Triplet Attention."""
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        scale = torch.sigmoid(self.bn(self.conv(self.compress(x))))
        return x * scale


class TripletAttention(nn.Module):
    """Triplet Attention for horizontal, vertical, and spatial/channel-aware refinement."""
...
class Bottleneck_TripletAttention(nn.Module):
    """Bottleneck block enhanced with Triplet Attention."""
...
class C3k_TripletAttention(C3):
    """C3k block using Bottleneck_TripletAttention."""

    def __init__(self, c1: int, c2: int, n: int = 1, shortcut: bool = True, g: int = 1, e: float = 0.5, k: int = 3):
        super().__init__(c1, c2, n, shortcut, g, e)
        c_ = int(c2 * e)
        self.m = nn.Sequential(*(Bottleneck_TripletAttention(c_, c_, shortcut, g, k=(k, k), e=1.0) for _ in range(n)))


class C3k2_TripletAttention(C2f):
    """C3k2 module enhanced with Triplet Attention, used near the detection head in MSAF-YOLO."""

    def __init__(self, c1: int, c2: int, n: int = 1, c3k: bool = False, e: float = 0.5, g: int = 1, shortcut: bool = True):
        super().__init__(c1, c2, n, shortcut, g, e)
        self.m = nn.ModuleList(
            C3k_TripletAttention(self.c, self.c, 2, shortcut, g) if c3k else Bottleneck_TripletAttention(self.c, self.c, shortcut, g)
            for _ in range(n)
        )


class EMSC(nn.Module):
    """Efficient Multi-Scale Convolution module.

    The input is split into an identity half and a multi-scale half. The multi-scale half
    is divided into two groups and processed by 3x3 and 5x5 depthwise convolutions.
    """
    ...
    def __init__(self, channels: int):
        ...
        self.dw3 = nn.Sequential(
            nn.Conv2d(g1, g1, 3, 1, 1, groups=g1, bias=False),
            nn.BatchNorm2d(g1),
            nn.SiLU(),
        )
        self.dw5 = nn.Sequential(
            nn.Conv2d(g2, g2, 5, 1, 2, groups=g2, bias=False),
            nn.BatchNorm2d(g2),
            nn.SiLU(),
        ) if g2 > 0 else nn.Identity()
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if x2.shape[1] == 0:
            return self.fuse(x)
        xa, xb = x2[:, :self.g1], x2[:, self.g1:]
        ya = self.dw3(xa)
        yb = self.dw5(xb) if xb.shape[1] > 0 else xb
        y2 = torch.cat((ya, yb), dim=1)
        return self.fuse(torch.cat((x1, y2), dim=1))


class Bottleneck_EMSC(nn.Module):
    """Bottleneck block enhanced with EMSC."""
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        y = self.emsc(self.cv2(self.cv1(x)))
        return x + y if self.add else y


class C3k_EMSC(C3):
    """C3k block using Bottleneck_EMSC."""
...
class C3k2_EMSC(C2f):
    """C3k2 module enhanced with EMSC, used in the MSAF-YOLO backbone."""
...
class CAKA(nn.Module):
    """Channel Aware Kernel Adaptor used in CAFM."""

    def __init__(self, channels: int, k: int = 3):
        super().__init__()
        m = 3 * k + 2
        self.square = nn.Conv2d(channels, channels, k, 1, k // 2, groups=channels, bias=False)
        self.hori = nn.Conv2d(channels, channels, (1, m), 1, (0, m // 2), groups=channels, bias=False)
        self.vert = nn.Conv2d(channels, channels, (m, 1), 1, (m // 2, 0), groups=channels, bias=False)
        self.weight = nn.Conv2d(channels, channels * 3, 1, 1, bias=True)
        self.bn = nn.BatchNorm2d(channels)
        self.act = nn.SiLU()

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        w = self.weight(F.adaptive_avg_pool2d(x, 1)).view(x.shape[0], 3, x.shape[1], 1, 1)
        w = torch.softmax(w, dim=1)
        ys = torch.stack((self.square(x), self.hori(x), self.vert(x)), dim=1)
        y = (ys * w).sum(dim=1)
        return self.act(self.bn(y))


class CAFM(nn.Module):
    """Channel-Aware Feature Mixer used to replace C2PSA in MSAF-YOLO."""
    ...
    def __init__(self, c1: int, c2: int):
        ...
        self.c_half = c_half
        self.c_rest = c2 - c_half
        self.caka5 = CAKA(c_half, k=5)
        self.caka3 = CAKA(self.c_rest, k=3) if self.c_rest > 0 else nn.Identity()
        self.out_proj = Conv(c2, c2, 1, 1)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        x = self.in_proj(x)
        x1, x2 = x[:, :self.c_half], x[:, self.c_half:]
        y1 = self.caka5(x1)
        y2 = self.caka3(x2) if x2.shape[1] > 0 else x2
        return self.out_proj(torch.cat((y1, y2), dim=1))


try:
    __all__ = tuple(list(__all__) + [
        "SPDConv", "ZPool", "TripletAttention", "Bottleneck_TripletAttention",
        "C3k_TripletAttention", "C3k2_TripletAttention", "EMSC", "Bottleneck_EMSC",
        "C3k_EMSC", "C3k2_EMSC", "CAKA", "CAFM",
    ])
except NameError:
    pass
# ========================= End MSAF-YOLO modules =========================


# ========================= WD-YOLO modules =========================
# Appended on top of ELA-YOLO + EL-YOLOv8 + MSAF-YOLO + SFW-YOLO modules.
# WD-YOLO uses a NeXt-style backbone and dual attention in the YOLO head.

class WDStem(nn.Module):
    """Patchify stem for WD-YOLO.

    The paper uses a non-overlapping convolution and LayerNorm to preserve fine-grained
    weld-image details while reducing resolution. For compatibility with the Ultralytics
    parser, c1 and c2 are supplied automatically by parse_model.
    """

    def __init__(self, c1: int, c2: int, k: int = 4, s: int = 4):
        super().__init__()
        self.conv = nn.Conv2d(c1, c2, kernel_size=k, stride=s, padding=0, bias=False)
...
class WDChannelShuffle(nn.Module):
    """Channel shuffle used after grouped convolution in WD-YOLO NeXt-Conv."""
...
class WDDownsample(nn.Module):
    """LayerNorm + non-overlapping convolution downsampling for WD-YOLO."""

    def __init__(self, c1: int, c2: int):
        super().__init__()
        self.norm = LayerNorm2d(c1)
        self.conv = nn.Conv2d(c1, c2, kernel_size=2, stride=2, padding=0, bias=False)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        return self.conv(self.norm(x))


class WDNeXtConv(nn.Module):
    """NeXt-Conv layer: 7x7 grouped conv + channel shuffle + inverted PW convolutions."""

    def __init__(self, c: int, groups: int = 4, expansion: int = 4):
        super().__init__()
        g = min(groups, c)
        while c % g != 0 and g > 1:
            g -= 1
        self.gconv = nn.Conv2d(c, c, kernel_size=7, stride=1, padding=3, groups=g, bias=False)
        self.shuffle = WDChannelShuffle(g)
...
class WDNeXtStage(nn.Module):
    """A WD-YOLO NeXt block stage with optional downsampling followed by NeXt-Conv layers."""

    def __init__(self, c1: int, c2: int, n: int = 3, downsample: bool = True, groups: int = 4, expansion: int = 4):
        super().__init__()
        self.down = WDDownsample(c1, c2) if downsample else (Conv(c1, c2, 1, 1) if c1 != c2 else nn.Identity())
        self.blocks = nn.Sequential(*(WDNeXtConv(c2, groups=groups, expansion=expansion) for _ in range(int(n))))

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        return self.blocks(self.down(x))


class WDBottleneck_CBAM(nn.Module):
    """YOLOv5-style bottleneck followed by CBAM for medium/large WD-YOLO heads."""
...
class C3_CBAM(nn.Module):
    """C3 block with CBAM bottlenecks, used by WD-YOLO for mid/large detection levels."""
    ...
    def __init__(self, c1: int, c2: int, n: int = 1, shortcut: bool = True, g: int = 1, e: float = 0.5):
        ...
        self.m = nn.Sequential(*(WDBottleneck_CBAM(c_, c_, shortcut, g, e=1.0) for _ in range(int(n))))

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        return self.cv3(torch.cat((self.m(self.cv1(x)), self.cv2(x)), dim=1))


class WDBRA(nn.Module):
    """Lightweight Bi-Level Routing Attention approximation for WD-YOLO small-object layer.

    The original BRA performs sparse routed attention between windows. This implementation
    keeps the same purpose while remaining dependency-free and stable in Ultralytics: it
    combines local depthwise filtering with global routing weights obtained from pooled
    context and a small channel gate.
    """
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        x = self.proj(x)
        y = self.local(x)
        c_gate = self.channel_gate(y)
        avg = torch.mean(y, dim=1, keepdim=True)
        mx = torch.amax(y, dim=1, keepdim=True)
        s_gate = self.spatial_gate(torch.cat((avg, mx), dim=1))
        return x + y * c_gate * s_gate


try:
    __all__ = tuple(list(__all__) + [
        "WDStem", "WDChannelShuffle", "WDDownsample", "WDNeXtConv", "WDNeXtStage",
        "WDBottleneck_CBAM", "C3_CBAM", "WDBRA",
    ])
except NameError:
    pass
# ========================= End WD-YOLO modules =========================
...
# ----------------------------- YoloTransformer-TransDetect modules -----------------------------

class TransDetectSwinBlock(nn.Module):
    """Lightweight local-window transformer block for BCHW feature maps."""
    def __init__(self, c: int, num_heads: int = 4, window_size: int = 7, mlp_ratio: float = 4.0, shift: bool = False):
        super().__init__()
        self.window_size, self.shift = window_size, shift
        heads = max(1, min(num_heads, c))
    ...
    def _reverse(self, windows, b, h, w, hp, wp, ws):
        c = windows.shape[-1]
        x = windows.view(b, hp // ws, wp // ws, ws, ws, c).permute(0, 1, 3, 2, 4, 5).contiguous()
        return x.view(b, hp, wp, c)[:, :h, :w, :].contiguous()
    ...
    def forward(self, x):
        ...
        if self.shift:
            y = torch.roll(y, shifts=(-ws // 2, -ws // 2), dims=(1, 2))
        win, oh, ow, hp, wp = self._partition(y, ws)
        z = self.norm1(win)
        z, _ = self.attn(z, z, z, need_weights=False)
        win = win + z
        win = win + self.mlp(self.norm2(win))
        y = self._reverse(win, b, oh, ow, hp, wp, ws)
        if self.shift:
            y = torch.roll(y, shifts=(ws // 2, ws // 2), dims=(1, 2))
...
class CNNSwinTransformer(nn.Module):
    """CNN-Swin Transformer fusion module."""
    def __init__(self, c1: int, c2: int, n: int = 1, heads: int = 4, window_size: int = 7):
        super().__init__()
        c_ = max(c2 // 2, 16)
        self.cv1, self.cv2 = Conv(c1, c_, 1, 1), Conv(c1, c_, 1, 1)
        self.swin = nn.Sequential(*(TransDetectSwinBlock(c_, heads, window_size, shift=bool(i % 2)) for i in range(max(1, n))))
        self.cv3 = Conv(2 * c_, c2, 1, 1)
    def forward(self, x):
        return self.cv3(torch.cat((self.swin(self.cv1(x)), self.cv2(x)), 1))
...
class WeightedConcat(nn.Module):
    ...
    def forward(self, xs):
        w = F.relu(self.w)
        w = w / (w.sum() + self.eps)
        return torch.cat([x * w[i] for i, x in enumerate(xs)], 1)


class WELAN(nn.Module):
    """Weighted ELAN module with learnable branch weights."""
    def __init__(self, c1: int, c2: int, n: int = 1, e: float = 0.5):
        super().__init__()
        c_ = max(int(c2 * e), 16)
        self.cv1, self.cv2 = Conv(c1, c_, 1, 1), Conv(c1, c_, 1, 1)
        self.blocks = nn.ModuleList(Conv(c_, c_, 3, 1) for _ in range(max(1, n) + 1))
        self.wcat = WeightedConcat(len(self.blocks) + 1)
        self.cv3 = Conv((len(self.blocks) + 1) * c_, c2, 1, 1)
    def forward(self, x):
        y, z = [self.cv1(x)], self.cv2(x)
        for m in self.blocks:
            z = m(z)
            y.append(z)
        return self.cv3(self.wcat(y))


class MsCS(nn.Module):
    """Multi-scale channel split/gating module."""
    def __init__(self, c1: int, c2: int):
        super().__init__()
        self.proj = Conv(c1, c2, 1, 1) if c1 != c2 else nn.Identity()
        c_ = max(c2 // 4, 16)
        self.branches = nn.ModuleList(nn.Sequential(nn.AdaptiveAvgPool2d(p), Conv(c2, c_, 1, 1)) for p in (1, 2, 4, 8))
        self.fuse = nn.Conv2d(c_ * 4, c2, 1, 1)
        self.out = Conv(c2, c2, 1, 1)
    def forward(self, x):
        x = self.proj(x)
        h, w = x.shape[-2:]
        y = torch.cat([F.interpolate(b(x), size=(h, w), mode='nearest') for b in self.branches], 1)
        return self.out(x * torch.sigmoid(self.fuse(y)) + x)


class CatConv(nn.Module):
    """Concatenated convolution downsampling block."""
    def __init__(self, c1: int, c2: int, k: int = 3, s: int = 2):
        super().__init__()
        c_ = c2 // 2
        self.b1 = nn.Sequential(Conv(c1, c_, 1, 1), Conv(c_, c_, k, s))
        self.b2 = Conv(c1, c2 - c_, k, s)
        self.out = Conv(c2, c2, 1, 1)
    def forward(self, x):
        return self.out(torch.cat((self.b1(x), self.b2(x)), 1))


# -----------------------------------------------------------------------------
# YOLO V5-IMPROVEMENT additions
# Paper components: Coordinate Attention (CA), FReLU activation, and SIoU loss.
# The SIoU loss is implemented in ultralytics/utils/loss.py and is activated only
# by YOLOV5ImprovementDetect. These network modules are safe to coexist with the
# ELA/EL/MSAF/SFW/WD/YoloTransformer-TransDetect modules above.
# -----------------------------------------------------------------------------
class FReLU(nn.Module):
    """Funnel ReLU activation for spatially aware nonlinear activation."""

    def __init__(self, c1: int, k: int = 3):
        super().__init__()
        self.conv = nn.Conv2d(c1, c1, k, 1, k // 2, groups=c1, bias=False)
...
class ConvFReLU(nn.Module):
    """Conv-BN-FReLU block used by YOLO V5-IMPROVEMENT."""
...
class CoordAtt(nn.Module):
    """Coordinate Attention module for long-range horizontal/vertical position encoding."""
...
__all__ = tuple(list(__all__) + ["FReLU", "ConvFReLU", "CoordAtt"]) if "__all__" in globals() else (
    "FReLU", "ConvFReLU", "CoordAtt"
)


# -----------------------------------------------------------------------------
# Context- and Scale-aware YOLO additions
# Paper components: PDD/CIA/SDD. PDD is implemented as optional offline ROI/text
# preprocessing. CIA is implemented here as a differentiable PPNet + DIP module.
# SDD is implemented in YAML as a unified P2-P6 multi-scale detection topology
# with a C3TR contextual block. The full twin-model WBF ensemble described in the
# paper can be run by training two YAML variants separately, but this merged
# version keeps one stable trainable model in the shared Ultralytics codebase.
# -----------------------------------------------------------------------------
class CSAPPNet(nn.Module):
    """Parameter prediction network for context-aware image adjustment."""

    def __init__(self, c1: int = 3, hidden: int = 256):
        super().__init__()
        self.features = nn.Sequential(
            nn.Conv2d(c1, 16, 3, 2, 1, bias=False),
            nn.BatchNorm2d(16),
...
class ContextAwareDIP(nn.Module):
    """Differentiable brightness/contrast image adjustment for Context and Scale-aware YOLO."""

    def __init__(self, c1: int = 3, hidden: int = 256, eps: float = 1e-6):
        super().__init__()
        self.ppnet = CSAPPNet(c1, hidden)
        self.eps = eps

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # Keep PPNet input dtype consistent with its parameters.
        # During Ultralytics validation/export, the model may be converted to FP16,
        # so forcing x.float() causes: input FloatTensor vs weight HalfTensor.
        pp_dtype = next(self.ppnet.parameters()).dtype
        pp_device = next(self.ppnet.parameters()).device
        x_pp = x.to(device=pp_device, dtype=pp_dtype)

        params = self.ppnet(x_pp)
        lam_b = (0.5 + 1.5 * params[:, 0]).view(-1, 1, 1, 1).to(dtype=x.dtype, device=x.device)
        lam_c = (0.5 + 1.5 * params[:, 1]).view(-1, 1, 1, 1).to(dtype=x.dtype, device=x.device)

        x_safe = torch.clamp(x, min=self.eps, max=1.0)
        xb = torch.pow(x_safe, lam_b)
        mean = xb.mean(dim=(2, 3), keepdim=True)
        xc = lam_c * xb + (1.0 - lam_c) * mean
        return torch.clamp(xc, 0.0, 1.0)


CSAContextAdjust = ContextAwareDIP
ContextScaleDIP = ContextAwareDIP

__all__ = tuple(list(__all__) + [
    "CSAPPNet", "ContextAwareDIP", "CSAContextAdjust", "ContextScaleDIP"
]) if "__all__" in globals() else (
    "CSAPPNet", "ContextAwareDIP", "CSAContextAdjust", "ContextScaleDIP"
)
