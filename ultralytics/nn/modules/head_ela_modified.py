...
__all__ = "OBB", "Classify", "Detect", "ELADetect", "ELAYOLOEDetect", "Pose", "RTDETRDecoder", "Segment", "YOLOEDetect", "YOLOESegment", "v10Detect"
...
class ELAYOLOEDetect(YOLOEDetect):
    """YOLOE-compatible ELA lightweight head.

    This keeps the YOLOE text-embedding classification interface but replaces the box and visual embedding branches
    with the same lightweight dilated-convolution idea. Use only if you want to keep YOLOE semantics; strict ELA-YOLO
    reproduction should use ELADetect.
    """
    def __init__(self, nc: int = 80, embed: int = 512, with_bn: bool = False, ch: tuple = ()):  # ch=(P3,P4,P5)
        super().__init__(nc, embed, with_bn, ch)
        c2 = max((16, ch[0] // 4, self.reg_max * 4))
        c3 = max(ch[0], min(self.nc, 100))
        self.cv2 = nn.ModuleList(ELADetectBranch(x, c2, 4 * self.reg_max, dilation=2) for x in ch)
        # output embed channels for BNContrastiveHead/ContrastiveHead
        self.cv3 = nn.ModuleList(ELADetectBranch(x, c3, embed, dilation=2) for x in ch)

    def bias_init(self):
        m = self
        for a, b, c, s in zip(m.cv2, m.cv3, m.cv4, m.stride):
            a.pred.bias.data[:] = 1.0
            b.pred.bias.data[:] = 0.0
